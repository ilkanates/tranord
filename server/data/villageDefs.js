...
const VILLAGE_DEFS = {
  ...
  */
  ...
  zirh:         { requires:[{ tip:"demirci", seviye:3}, {tip:"runSalonu", seviye:1 }], cpPerLevel:1, name:'Zırhçı',           category:'askeri',   icon:'🛡️', description:'Külçe demirden ZIRH ve KALKAN döver — savunma tarafının teçhizatı. Atanan işçi sayısı üretim hızını belirler, seviye ise kaç işçi alabileceğini (seviye başına 3). Ürettiğin parçalar Cephanelikteki ORTAK havuzda durur. Ekipmanın SEVİYESİNİ de burada yükseltirsin: zırh ve kalkan seviyesi bütün savunan birliklerine birden işler, tek tek askere değil.',
                   ...
                   buildBaseWork:25, buildMultiplier: 1.28, cost:{ kereste:45, yontmaTas:35, demirKulce:35 },
                   upgradeCostBase:{ kereste:45, yontmaTas:35, demirKulce:35 }, upgradeCostMultiplier: 1.28 },
  ...
  ahir:         { requires:[{ tip:"kisla", seviye:3}, {tip:"runSalonu", seviye:3}, {tarla:"tahil", seviye:5 }], cpPerLevel:1, name:'Ahır',             category:'askeri',   icon:'🐎', description:'Süvari birliklerini eğitir ve AT besler. Seviye × 5 kadar at tutabilir — at yoksa süvari eğitilemez, çünkü her süvari bir at tüketir. Atlar ayrıca günde 3 HAM TAHIL yer (un ya da ekmek değil), yani büyük bir süvari ordusu tarlalarını doğrudan ezer. Seviye başına 3 işçi alır; işçi sayısı eğitim süresini kısaltır.',
                   unique:true,  maxLevel:20, workersPerLevel:3, horseCapPerLevel: 20,
                   buildBaseWork:35, buildMultiplier: 1.28, cost:{ kereste:60, tahil:60 },
                   upgradeCostBase:{ kereste:60, tahil:60 }, upgradeCostMultiplier: 1.28 },
  ...
  kisla:        { requires:[{ tip:"anaBina", seviye:3}, {tip:"runSalonu", seviye:1 }], cpPerLevel:1, name:'Kışla',            category:'askeri',   icon:'🛡️', description:'Piyade birliklerini eğitir. Seviyesi hangi birimleri eğitebileceğinin kapısıdır (Rún Salonu araştırmasıyla birlikte), atanan işçi sayısı (seviye başına 3) eğitim süresini kısaltır. Eğitim sırası tek kuyruk hâlinde ilerler; sıradaki birim için malzeme yoksa kuyruk bekler. Asker ayrıca nüfustan yer ve günde 6 ekmek yer — köylünün iki katı.',
                   unique:true,  maxLevel:20, workersPerLevel:3,
                   buildBaseWork:35, buildMultiplier: 1.28, cost:{ kereste:60, yontmaTas:70 },
                   upgradeCostBase:{ kereste:60, yontmaTas:70 }, upgradeCostMultiplier: 1.28 },
  atolye:       { requires:[{ tip:"anaBina", seviye:5}, {tip:"demirci", seviye:5}, {tip:"runSalonu", seviye:5 }], cpPerLevel:1, name:'Atölye',           category:'askeri',   icon:'🏗️', description:'Koç başı ve mancınık üretir. İkisi ORTAK bir kapasiteyi paylaşır: seviye × 5 makine, yani Lvl 5 atölyede toplam 25. Koç başı YALNIZ suru kırar (hendeğe dokunmaz, sur sıfırlanınca artan güç boşa gider); mancınık ise seçtiğin BİNAYI yıkar. İkisi de tek başına savaşmaz, orduyla gider ve savaşı kazanman gerekir — kaybeden ordunun kuşatması işlemez. Kuşatma makineleri Cephanelik havuzunu KULLANMAZ; onlar makine, kılıç değil. Seviye başına 3 işçi alır.',
                   ...
                   buildBaseWork:30, buildMultiplier: 1.28, cost:{ kereste:75, demirKulce:75 },
                   upgradeCostBase:{ kereste:75, demirKulce:75 }, upgradeCostMultiplier: 1.28 },
  cephane:      { requires:[{ biri:["silahci", "zirh"], seviye:1 }], cpPerLevel:1, name:'Cephanelik',       category:'askeri',   icon:'🏹', description:'Kılıç, mızrak, kalkan ve zırh tek bir ORTAK havuzu paylaşır ve o havuzun büyüklüğünü bu bina belirler: seviye başına +200 yer. Cephaneliğin yoksa havuz yalnız 80 parçadır — ilk ordudan sonra tıkanır. Havuz ortak olduğu için tek bir tür (mesela kılıç) havuzun tamamını doldurabilir; ne üreteceğine sen karar verirsin. Atlar ve kuşatma makineleri bu havuza girmez.',
                   ...
                   buildBaseWork:30, buildMultiplier: 1.28, cost:{ kereste:60, yontmaTas:70, demirKulce:35 },
                   upgradeCostBase:{ kereste:60, yontmaTas:70, demirKulce:35 }, upgradeCostMultiplier: 1.28 },
  saglikCadiri: { requires:[{ tip:"kisla", seviye:3}, {tip:"firin", seviye:3 }], cpPerLevel:1, name:'Sağlık Çadırı',    category:'askeri',   icon:'⛺', description:'Köyün SAVUNULDUĞU savaşlarda ölen askerlerinin bir kısmı yaralı sayılıp çadıra alınır: her seviye %2, Lvl 20 de %40. Tedavi KENDİLİĞİNDEN başlamaz — hangi birliği ayağa kaldıracağına sen karar verirsin; seçtiğin birlik kendi eğitim süresinin 2 katı kadar sürede iyileşip orduna döner. Yatak sayısı seviye × 10 (Lvl 20 de 200) ve yataklar tedavi bitene kadar doludur; sığmayan yaralı ölür. Saldırıda ölen askere işlemez — çadır köyde. Köyünde duran misafir askerlere de işlemez.',
                   unique:true,  maxLevel:20, buildBaseWork:25, buildMultiplier: 1.28, cost:{ kereste:35, tahil:30 } },
  /*
...
/**
 * NÜFUS TAVANI — evsiz köyün taban kapasitesi.
 * Tavan = BASE_POPULATION + Σ (ev seviyesi × ev.populationPerLevel).
 * Bu iki sayı üç ayrı yerde elle yazılıydı; tek kaynak burası.
 */
...
function maxPopulationOf(village) {
  ...
  let sum = BASE_POPULATION;
  for (const b of Object.values(village.villageBuildings || {})) {
    if (b?.type !== 'ev') continue;
    if (b.building && b.level === 0) continue;
    sum += perLevel * (b.level || 0);
  }
  return sum;
}
