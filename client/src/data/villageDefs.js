 ...
 */
export const UPGRADE_MULT_DEFAULT = 1.28;

/**
 * ÖN KOŞUL DENETİMİ — sunucudaki game/insaat.js · eksikOnKosullar ile
 * AYNI kural. İkizi burada tutmanın sebebi menünün kilidi ve sebebini
 * sunucuya sormadan gösterebilmesi: aksi hâlde oyuncu düğmeye basar,
 * hiçbir şey olmaz ve sebebini ancak sunucu cevabında görürdü.
 */
function enYuksekBina(binalar, tip) {
  let en = 0;
  for (const b of Object.values(binalar || {})) if (b?.type === tip && b.level > en) en = b.level;
  return en;
}

function enYuksekTarla(tarlalar, tip) {
  let en = 0;
  for (const t of Object.values(tarlalar || {})) if (t?.type === tip && t.level > en) en = t.level;
  return en;
}

export function eksikOnKosullar(binalar, tarlalar, tip, tarlaAdlari = {}) {
  const liste = VILLAGE_DEFS[tip]?.requires || [];
  const eksik = [];
  for (const k of liste) {
    const gereken = k.seviye || 1;
    if (k.tarla) {
      if (enYuksekTarla(tarlalar, k.tarla) < gereken) {
        eksik.push(`${tarlaAdlari[k.tarla] || k.tarla} Lvl ${gereken}`);
      }
      continue;
    }
    if (k.biri) {
      if (!k.biri.some(t => enYuksekBina(binalar, t) >= gereken)) {
        eksik.push(k.biri.map(t => VILLAGE_DEFS[t]?.name || t).join(' ya da ') + ` Lvl ${gereken}`);
      }
      continue;
    }
    if (enYuksekBina(binalar, k.tip) < gereken) {
      eksik.push(`${VILLAGE_DEFS[k.tip]?.name || k.tip} Lvl ${gereken}`);
    }
  }
  return eksik;
}
...
const VILLAGE_DEFS = {
  ...
  anaBina: { cpPerLevel:2,
    ...
    buildBaseWork:50, buildMultiplier: 1.28,
  ...
  /*
    ...
    tarlalar bedava üretiyor. Değirmen ve fırın kuralın dışında — un
    ve ekmek inşaatı kilitlemiyor, yalnız nüfusu besliyor.
  ...
  */
  degirmen: { requires:[{ tarla:"tahil", seviye:1 }], cpPerLevel:1,
    ...
    unique:true, maxLevel:20, workersPerLevel:5,
    processes:{ input: 'tahil', inputPerHour: 120, output: 'un', outputPerHour: 96 },
    buildBaseWork:20, buildMultiplier: 1.28, cost:{ odun:90, tas:30 }
  },
  firin: { requires:[{ tip:"degirmen", seviye:3 }], cpPerLevel:1,
  ...
  elcilik: { requires:[{ tip:"anaBina", seviye:3 }], cpPerLevel:2,
    ...
    description:'BİRLİĞİN MERKEZİ. Elçiliği olan oyuncu kendi birliğini kurabilir; adını ve amblemini seçer, sonra oyuncu adı aratıp davet gönderir. SEVİYE ÜYE TAVANIDIR: her seviye 3 üye, yani Lvl 1 de üç kişilik bir çete, Lvl 20 de 60 kişilik bir hanedan — tavanı birliğin KONUNG unun elçiliği belirler, üyelerinkiler toplanmaz. Dört sekme var: ÜYELER (kim çevrimiçi, kimin kaç köyü ve kaç savaş puanı var), DİPLOMASİ (konfederasyon ve saldırmazlık teklifi, savaş ilanı, birlik sıralaması), GÜNLÜK (kim katıldı, kim çıkarıldı, hangi savaşı kim ilan etti) ve PROFİL (birliğin tanıtım metni ve toplam gücü). Diplomasi hamlelerini yalnız Konung yapar. Birlik üyelerinin toprağı haritada yeşil çerçeveyle görünür ve komşu iki üyenin arasına koyu yeşil sınır çizilir. NE BİRLİK NE DE ANLAŞMA SALDIRIYA KARŞI KORUMADIR: birlik arkadaşına da, saldırmazlık imzaladığın birliğe de saldırmak serbesttir — anlaşmanın bedeli itibardır, oyunun kuralı değil.',
    unique:true, maxLevel:20,
  ...
  hammaddeDepo: { requires:[{ tip:"anaBina", seviye:1 }], cpPerLevel:1,
    ...
    buildBaseWork:30, buildMultiplier: 1.28, cost:{ kereste:60, yontmaTas:60, tugla:45 }
  },
  islenmisMalDepo: { requires:[{ biri:["keresteci", "tuglaci", "tasci", "demirci", "degirmen", "firin"], seviye:2 }], cpPerLevel:1,
  ...
  */
  ...
  // ── Ekonomik ────────────────────────────────────────────────────
  pazar:      { requires:[{ tip:"anaBina", seviye:3}, {tip:"hammaddeDepo", seviye:2 }], cpPerLevel:2, name:'Pazar',              category:'ekonomik', icon:'🏪', description:'Hem NPC ile takas, hem oyuncular arası teklif, hem de KENDİ köylerine kaynak gönderme buradan olur. Seviyesi TÜCCAR SAYISIDIR: her seviye 1 tüccar, her tüccar 2000 kaynak taşır. Açık bir teklif ya da yoldaki bir sevkiyat tüccarı bağlar; teklifi iptal edersen tüccar geri döner. Tüccarlar yürür, ışınlanmaz — uzak köye gönderi zaman alır. Pazar yükseltilirken de çalışmaya devam eder.',                              unique:true, maxLevel:20, buildBaseWork:40, buildMultiplier: 1.28, cost:{ kereste:60, yontmaTas:95 } },
  loncaDemir: { requires:[{ tip:"demirci", seviye:5 }], cpPerLevel:2, name:'Demirciler Loncası', category:'ekonomik', icon:'🔩', description:'Köyün DEMİR tarlalarının üretimini artırır: her seviye +%5, en fazla 5 seviye, yani +%25. Bonus tarlaların üretimine çarpan olarak işler — tarlan yoksa artıracak bir şey de yoktur. Demir oyunun en dar kaynağı olduğu için bu lonca genelde ilk kurulanıdır.', unique:true, maxLevel:5, bonusPerLevel:5, affects:'demir',  buildBaseWork:30, buildMultiplier: 1.28, cost:{ kereste:50, demirKulce:90 } },
  loncaOdun:  { requires:[{ tip:"keresteci", seviye:5 }], cpPerLevel:2, name:'Oduncular Loncası',  category:'ekonomik', icon:'🪓', description:'Köyün ORMAN tarlalarının üretimini artırır: her seviye +%5, en fazla 5 seviye (+%25). Bonus tarla üretimine çarpan olarak işler; işleme binalarına (keresteci) doğrudan dokunmaz — daha çok odun, dolayısıyla daha çok kereste demektir.',  unique:true, maxLevel:5, bonusPerLevel:5, affects:'odun',   buildBaseWork:30, buildMultiplier: 1.28, cost:{ kereste:60, yontmaTas:45 } },
  loncaTas:   { requires:[{ tip:"tasci", seviye:5 }], cpPerLevel:2, name:'Taşçılar Loncası',   category:'ekonomik', icon:'⛏️', description:'Köyün TAŞ tarlalarının üretimini artırır: her seviye +%5, en fazla 5 seviye (+%25). Sur, kule ve askeri binalar yontma taş yediği için savunmaya oynayan bir köyde erken kurulur.',   unique:true, maxLevel:5, bonusPerLevel:5, affects:'tas',    buildBaseWork:30, buildMultiplier: 1.28, cost:{ kereste:50, yontmaTas:70 } },
  loncaKil:   { requires:[{ tip:"tuglaci", seviye:5 }], cpPerLevel:2, name:'Kilciler Loncası',   category:'ekonomik', icon:'🟫', description:'Köyün KİL tarlalarının üretimini artırır: her seviye +%5, en fazla 5 seviye (+%25). Kil, tuğla üzerinden yönetim binalarına (köşk, saray, taverna) gider — yani genişlemeye oynayan köyün loncasıdır.',   unique:true, maxLevel:5, bonusPerLevel:5, affects:'kil',    buildBaseWork:30, buildMultiplier: 1.28, cost:{ kereste:50, tugla:90 } },
  loncaTahil: { requires:[{ tip:"degirmen", seviye:5 }], cpPerLevel:2, name:'Tahılcılar Loncası', category:'ekonomik', icon:'🌾', description:'Köyün TAHIL tarlalarının üretimini artırır: her seviye +%5, en fazla 5 seviye (+%25). Tahıl hem nüfusu hem orduyu hem atları besler; büyük ordusu olan köyde bu lonca ordunun büyüklüğünü doğrudan belirler.', unique:true, maxLevel:5, bonusPerLevel:5, affects:'tahil',  buildBaseWork:30, buildMultiplier: 1.28, cost:{ kereste:50, tahil:60 } },
  ...
  kosk: { requires:[{ tip:"anaBina", seviye:5}, {tip:"taverna", seviye:1 }],
  ...
  saray: { requires:[{ tip:"anaBina", seviye:8}, {tip:"taverna", seviye:3 }],
    ...
    buildBaseWork:60, buildMultiplier: 1.28,
  ...
  taverna: { requires:[{ tip:"anaBina", seviye:5}, {tip:"firin", seviye:3 }],
    ...
    unique:true, maxLevel:20, cpPerLevel:3,
    festival:true,
    buildBaseWork:35, buildMultiplier: 1.28,
