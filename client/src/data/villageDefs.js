...
const VILLAGE_DEFS = {
  ...
  saglikCadiri: { cpPerLevel:1, name:'Sağlık Çadırı',   category:'askeri', icon:'⛺', description:'Köyün SAVUNULDUĞU savaşlarda ölen askerlerin bir kısmı yaralı sayılır ve çadıra alınır; eğitim süresinin 2 katı kadar sürede iyileşip orduna döner. Her seviye %2 yaralı payı ve 10 yatak, Lvl 20 de %40 ve 200 yatak. Yatak dolarsa fazla yaralı ölür. Saldırıda ölen askere işlemez — çadır köyde.',             unique:true, maxLevel:20, buildBaseWork:25, buildMultiplier:1.7, cost:{ kereste:35, tahil:30 } },
  // Kahramanın evi — doğduğu, iyileştiği ve eşyalarını tuttuğu yer.
  // İKİZ TANIM: sunucudaki villageDefs.js ile birlikte değişir.
  kahramanKonagi: { cpPerLevel:2, name:'Kahraman Konağı', category:'askeri', icon:'🏅', description:'Kahramanın evi: burada doğar, ölünce burada dirilir, eşyalarını burada tutar. HER SEVİYE üç şey veriyor — iyileşme hızı +1,5 can/sa, macera tavanı (iki seviyede bir +1, Lvl 20 de 12), ve yeni macera bekleme süresi (6 sa → 2,2 sa). Ayrıntılar Kahraman ekranında yazılı.', unique:true, maxLevel:20, workersPerLevel:1, buildBaseWork:35, buildMultiplier:1.85, cost:{ kereste:70, yontmaTas:90, demirKulce:50 }, upgradeCostBase:{ kereste:70, yontmaTas:90, demirKulce:50 }, upgradeCostMultiplier:1.7 },
  ...
  // ── Ekonomik ────────────────────────────────────────────────────
  pazar:      { cpPerLevel:2, name:'Pazar',              category:'ekonomik', icon:'🏪', description:'Hammadde al-sat.',                              unique:true, maxLevel:20, buildBaseWork:40, buildMultiplier:1.8, cost:{ kereste:60, yontmaTas:95 } },
  loncaDemir: { cpPerLevel:2, name:'Demirciler Loncası', category:'ekonomik', icon:'🔩', description:'Demir üretimini artırır. +%5/seviye. Maks 5.', unique:true, maxLevel:5, bonusPerLevel:5, affects:'demir',  buildBaseWork:30, buildMultiplier:2.0, cost:{ kereste:50, demirKulce:90 } },
  loncaOdun:  { cpPerLevel:2, name:'Oduncular Loncası',  category:'ekonomik', icon:'🪓', description:'Odun üretimini artırır. +%5/seviye. Maks 5.',  unique:true, maxLevel:5, bonusPerLevel:5, affects:'odun',   buildBaseWork:30, buildMultiplier:2.0, cost:{ kereste:60, yontmaTas:45 } },
  loncaTas:   { cpPerLevel:2, name:'Taşçılar Loncası',   category:'ekonomik', icon:'⛏️', description:'Taş üretimini artırır. +%5/seviye. Maks 5.',   unique:true, maxLevel:5, bonusPerLevel:5, affects:'tas',    buildBaseWork:30, buildMultiplier:2.0, cost:{ kereste:50, yontmaTas:70 } },
  loncaKil:   { cpPerLevel:2, name:'Kilciler Loncası',   category:'ekonomik', icon:'🟫', description:'Kil üretimini artırır. +%5/seviye. Maks 5.',   unique:true, maxLevel:5, bonusPerLevel:5, affects:'kil',    buildBaseWork:30, buildMultiplier:2.0, cost:{ kereste:50, tugla:90 } },
  loncaTahil: { cpPerLevel:2, name:'Tahılcılar Loncası', category:'ekonomik', icon:'🌾', description:'Tahıl üretimini artırır. +%5/seviye. Maks 5.', unique:true, maxLevel:5, bonusPerLevel:5, affects:'tahil',  buildBaseWork:30, buildMultiplier:2.0, cost:{ kereste:50, tahil:60 } },
  ...
  // ── Savunma ─────────────────────────────────────────────────────
  sur:    { cpPerLevel:1, name:'Sur',            category:'savunma', icon:'🏰', description:'Savunmacılara savunma bonusu. Maks Lvl 20.',  unique:true,  maxLevel:20, buildBaseWork:50, buildMultiplier:2.0, bonusTable:SUR_BONUS, cost:{ yontmaTas:190, kereste:25 } },
  hendek: { cpPerLevel:1, name:'Hendek',         category:'savunma', icon:'〰️', description:'Sur ile birleşik, yarı bonus. Maks Lvl 20.', unique:true,  maxLevel:20, buildBaseWork:40, buildMultiplier:1.9, bonusTable:HENDEK_BONUS, cost:{ kereste:25, yontmaTas:95 } },
  kule:   { cpPerLevel:1, name:'Savunma Kulesi', category:'savunma', icon:'🗼', description:'Kuleye atanan askerlere iki kat bonus.',      unique:false, maxLevel:20, buildBaseWork:45, buildMultiplier:2.0, bonusTable:KULE_BONUS, maxInstances:6, workersPerLevel:4, cost:{ kereste:50, yontmaTas:95, demirKulce:75 } }
};
